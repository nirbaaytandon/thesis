...
\begin{center}
	\begin{tikzpicture}[auto,
		...
		block_center/.style ={rectangle, draw=black, thick, fill=white,
			text width=8em, text badly centered,font=\small},
		block_left/.style ={rectangle, draw=black, thick, fill=white,
			text width=18em, text badly centered,inner sep=0.2em,font=\small},
		line/.style ={draw, thick, -latex', shorten >=0pt}]
		% outlining the flowchart using the PGF/TikZ matrix funtion
		\matrix [column sep=5mm,row sep=3mm] {
			% enrollment - row 1
			\node [block_center] (introduction) {Introduction};
		...
		\end{itemize}}; \\
			% enrollment - row 2
			\node [block_center] (litreview) {Literature Review}; 
		...
		\end{itemize}}; \\
						% enrollment - row 3
			\node [block_center] (researchMeth) {Research Methodology};
			...
			\end{itemize}}; \\
			% enrollment - row 2
			\node [block_center] (dc) {Data Collection}; 
			& \node [block_left] (dcleft) {\begin{itemize}
					\item Review sample datasets
					\item Finalize dataset
			\end{itemize}}; \\
					% enrollment - row 1
		\node [block_center] (researchExperiments) {Research Experiments};
		...
		\end{itemize}}; \\
		% enrollment - row 2
		\node [block_center] (results) {Results}; 
