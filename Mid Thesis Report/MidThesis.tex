...
\citep{fastQA}
	    ...
	    \section{Transformers}\label{23}
           ...
           \end{equation}
            ...
            The advantages of the Transformer architecture are clearly evident over the standard RNN based LSTMs and other architectures. Experiments on the WMT 2014 Englist to German translation using the big transformer showed a significant jump in BLEU score 28.4 over various other seq2seq models and even combined ensemble models which earlier had a highest score of 26.36 using a ConvS2S Ensemble \citep{convs2s}.
		...
		\subsubsection{BERT}\label{2321}
			...
			A common theme with BERT is that it takes a long time to train. Especially in the BERT based RoBERTa architecture \cite{roberta}. RoBERTa takes on average 4-5 times more time to train than BERT, however, it also shows a maximum of 20\% improvement over BERT, depending on the application. ALBERT, which is A Lite BERT, \cite{albert} was developed specifically to deal with memory limitations and reduce training times. ALBERT's XXL implementation has been documented to perform better in models trained on the BOOKCORPUS and Wikipedia ones by at least 2\% across multiple applications. In a smaller amount of time.
...
%    mention datasets like,
		...
		The table \ref{datasetDescription} shows how much difference there is between SQuAD 1.1 and SQuAD 2.0 datasets.
    \section{Data Pre-processing And Transformation}\label{c32}
    Techniques used.
    (1) Average pooling; (2) max pooling; (3) the concatenation of the last vectors on both directions. 
    \section{Research Hypothesis}\label{c33}
         To use this dataset correctly we shall perform the following pre-processing steps on it with:
    ...
    \begin{enumerate}
		...
		\item One hot encoding the context before loading
		\item BERT base\citep{bert}
		\item Answer pointer output layer\citep{lstmPointer, lstmhu2016question} before output using softmax layer.
		
    \end{enumerate}

    \section{Existing Models And Benchmarks}\label{c34}
    To disscus, is this to be done here.
