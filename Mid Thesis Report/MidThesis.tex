...
\cite{fastQA}
	    ...
	    \section{Transformers}\label{23}
			...
			\textit{Transformers} were introduced in late 2017 by \citep{atayl} and have become a very reliable way to implement various NLP based tasks. 
			While \citep{atayl} used the BLEU score and showed that their model works better than state of the art models on the WMT 2014 dataset \citep{wmt} In this section we will critically evaluate a few key transformer based architectures \& assess their application particularly on the SQuAD 2.0 dataset.
